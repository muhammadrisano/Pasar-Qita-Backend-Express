...
module.exports = {
    getStore: () => {
        return new Promise((resolve, reject) => {
            connection.query("SELECT store.*, market.name_market, market.address, market.longitude, market.latitude, user.name, user.telp FROM store INNER JOIN market ON store.id_market = market.id_market INNER JOIN user ON store.shop_selector = user.id_user ", (err, result) => {
                if (!err) {
                    resolve(result)
                } else {
